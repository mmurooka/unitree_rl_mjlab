...
class MotionPromptLoader:
  ...
  def _load_motion(self):
    ...
    self.motion_base_poss_input = torch.from_numpy(q_ref[:, :3]).to(self.device)
    # Pinocchio free-flyer q uses [x, y, z, qx, qy, qz, qw].
    self.motion_base_rots_input = torch.from_numpy(q_ref[:, [6, 3, 4, 5]]).to(
      self.device
    )
    self.motion_dof_poss_input = torch.from_numpy(q_ref[:, 7:]).to(self.device)
...
def run_sim(
  ...
  while not file_saved:
    ...
    if not file_saved:
      ...
      log["foot_contact"].append(motion_contact[0].cpu().numpy().copy())
