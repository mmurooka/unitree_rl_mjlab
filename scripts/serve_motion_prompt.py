...
class MotionService:
    def __init__(self, converter, deploy_endpoint, *, save_debug=True,
                 debug_dir="/tmp/task_prompt_rl/received", dofs=29):
    ...
    def execute(self, input_path: Path, output_path: Path):
        try:
            ...
            if status != "READY":
                return {"status": "BUSY" if status == "BUSY" else "ERROR",
                        "message": status}
            ...
            return {"status": "STARTED" if status == "STARTED" else
                    "BUSY" if status == "BUSY" else "ERROR", "message": status}
            ...
            try:
                while not stop.is_set():
                    if pending is not None and pending[0].done():
                        ...
                        socket.send_multipart(address + [json.dumps(future.result()).encode()])
                    ...
                    temporary = None
                    if self.save_debug:
                        folder = Path(tempfile.mkdtemp(prefix="motion_", dir=self.debug_dir))
                    else:
                        temporary = tempfile.TemporaryDirectory(prefix="motion_prompt_", dir="/tmp")
                        folder = Path(temporary.name)
                    input_path, output_path = folder / "motion_prompt.npz", folder / "policy_input.npz"
                    ...
                    if pending is not None:
                        ...
                        if temporary is not None:
                            temporary.cleanup()
                        continue
                    pending = (executor.submit(self.execute, input_path, output_path),
                               address, temporary)
            finally:
                if pending is not None:
                    pending[0].result()
...
def main():
    ...
    service = MotionService(MotionPromptConverter("g1", args.device), args.deploy_endpoint,
                            save_debug=not args.no_save_debug, debug_dir=args.debug_dir)
    try:
